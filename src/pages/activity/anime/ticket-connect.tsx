...
import { Connect } from '@/features/Activity/Anime/Comps/Connect';
import { Pass } from '@/features/Activity/Anime/Comps/Pass';
...
const getTicketInfoUrl = '/api/project/v1/anime/tickets/get';
const prodRedeemPrefix =
  'https://unemeta.com/activity/anime/ticket-redeem?redeemInfo';
const testRedeemPrefix =
  'https://test.unemeta.com/activity/anime/ticket-redeem?redeemInfo';
...
export default function TicketConnect() {
  ...
  const [tipsWording, setTipsWording] = useState(
    'Here is no associated nft ticket under your current wallet, Please confirm and try again later',
  );

  function handleRedeem(checkedTicket: any) {
    console.info('[checkedTicket]:', checkedTicket);
    const checkedList: any[] = [];
    checkedTicket.forEach((item: any) =>
      redeemList.push({
    ...
    if (!mintList || (mintList as any).length === 0) {
      ...
      setTipsWording('Please select the ticket you want to redeem');
      confModalRef.current.open();
      return;
    }
