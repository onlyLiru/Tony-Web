...
import { CollectionHeader } from '@/features/CollectionPage';
import { useTranslations } from 'next-intl';
import { PageTabs } from '@/features/PageTabs';
import { NftFilterForm } from '@/features/FilterModule';
import { useIsRare } from '@/store';

function getList(p: any) {
  parseCommonContentRequestParams(p);
  return marketApis.collectionItemList({ ...p, page_size: 20 }).then((r) => ({
    list: r.data.item,
    noMore:
...
export default function CollectionDetail({
  ...
  return (
    <>
      <CommonHead
        title={collectionDetail.collection.name}
        description={collectionDetail.collection.description}
        image={collectionDetail.collection.logo}
      />
      <CollectionHeader collectionDetail={collectionDetail} />
      <PageTabs
        tabs={[
          { label: t('items'), href: `/collection/${query.chain}/${query.id}` },
          {
            label: t('activites.title'),
            href: `/collection/${query.chain}/${query.id}/activity?events[]=4`,
          },
        ]}
      />
      <FilterContainer.Provider>
        <NftFilterForm.Provider
          collectionAddress={query.id as string}
          chainId={+query.chain!}
        >
          <NftFilterCommonContent
            params={{ address: query.id as string, rare_mode: isRare }}
            request={getList}
          />
        </NftFilterForm.Provider>
      </FilterContainer.Provider>
...
export async function getServerSideProps({
  ...
  try {
    const { data } = await marketApis.collectionDetail({
      chain_id: +chain!,
      contract_address: id as string,
    });
