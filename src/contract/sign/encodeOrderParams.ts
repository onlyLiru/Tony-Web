 ...
 */
export const encodeOrderParams = (
  params?: any[] | null,
): { paramsTypes: SolidityType[]; encodedParams: BytesLike } => {
  const nonNullParams = params || [];
  const paramsTypes: SolidityType[] = nonNullParams.map(
    (param): SolidityType => {
  ...
  return {
    paramsTypes,
    encodedParams: utils.defaultAbiCoder.encode(paramsTypes, nonNullParams),
  };
};
