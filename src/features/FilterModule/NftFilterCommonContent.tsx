...
export const NftFilterCommonContent = observer(
  ({
    request,
    params = {},
  }: {
    ...
    const [curretnChainId] = useLocalStorage(LOCAL_KEY.currentChainIdKey);
    ...
    const {
      ...
      isNoMore: (d) => d?.noMore!,
      reloadDeps: [state, params, { chain_id: curretnChainId }],
      pageSize: 20,
      threshold: 248,
    });
    useEffect(() => {
      if (curretnChainId && nftStore.toRefreshAll) {
        reload();
        setTimeout(() => {
          nftStore.setToRefreshAll(false);
        }, 200);
      }
    ...
    const hideBuyButton = useCallback(
      (ownerAddress: string) => {
        // 3333333 hidden buy
        return toLower(userData?.wallet_address) === toLower(ownerAddress);
      },
