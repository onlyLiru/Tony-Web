...
const CollectionList = observer(
  (props: {
    ...
    handleItemClick?: (
      item: marketApis.ApiMarket.ExploreFilterCollectionItem | undefined,
    ) => void;
  }) => {
    const t = useTranslations('common');
    const { data, isEmpty, isLoading, loadMore, reload } = useInfiniteScroll(
      getList,
      {
    ...
    useEffect(() => {
      if (nftStore.toRefreshAll === true) {
        reload();
      }
    }, [nftStore?.toRefreshAll]);
...
const CollectionAttrList = (
  ...
  const [isRare] = useIsRare();
  const { loading, data, run } = useRequest(marketApis.collectionAttributes, {
    defaultParams: [
      {
        address: collectionAddress || state.address!,
  ...
  return (
    <Box w="full">
      ...
      {(() => {
        if (loading)
          return (
            <Center w="full" h="20vh" color={'rgba(255, 255, 255, 0.80)'}>
              <Spinner />
            </Center>
          );
        if (
          ...
          return (
            <Center h="20vh" fontWeight={'700'} fontFamily="Inter">
              <Text color={isRare ? 'white' : 'rgba(255, 255, 255, 0.80)'}>
                {t('noItems')}
              </Text>
            </Center>
          );
...
export const CollectionFilter = (props: CollectionFilterProps) => {
  ...
  const { visitChain, visitChainId } = useSwitchChain({});
  const [inputValue, setInputValue] = useState('');
  ...
  if (collectionAddress) return <CollectionAttrList {...props} />;

  return (
    <>
      <FormControl bg={isRare ? darkBg : ''}>
        <FormLabel
        ...
        {!!state.address ? (
          <CollectionAttrList collection={currentCollection} {...props} />
        ) : (
          <Box>
            <InputGroup w="full">
              <Input
              ...
              <InputRightElement
                ...
                children={
                  inputValue ? (
                    <CloseIcon w="10px" color="#b5b5b5" />
                  ) : (
                    <NavBoldSeachIcon color="#b5b5b5" />
                  )
                }
              />
            </InputGroup>
            ...
            <CollectionList
              ...
              params={{
                ...
                name_like: keyword?.trim(),
                owner_wallet_address: ownerWalletAddress,
              }}
              handleItemClick={(item) => {
                setCurrentCollection(item);
                dispatch((prev) => ({ ...prev, address: item?.address || '' }));
              }}
            />
          </Box>
        )}
